...
(`roborev init`), z-adversarial-review skill installed. `tools/check-pipeline.sh`
verifies all of it.
...
`roborev close <id>` and a one-line comment naming the fixing commit.
...
- Completion status per rules/WORKFLOW.md (DONE / DONE_WITH_CONCERNS /
  BLOCKED).
