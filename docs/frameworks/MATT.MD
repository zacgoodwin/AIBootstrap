...
All skills from [mattpocock/skills](https://github.com/mattpocock/skills), sorted by the same sections as the other files in docs/frameworks/. Skills not yet in the upstream README are marked **new**; in-progress skills are tagged (in-progress). Upstream not pinned in tools/sources.json; `/skills-update` records the commit on its first full catalog pass.
